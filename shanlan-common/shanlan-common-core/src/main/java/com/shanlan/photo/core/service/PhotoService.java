...
public class PhotoService {
     ...
     * （2）再在裁剪图的基础等比例压缩两张图，分别为200*200，120*120,32*32
     * （3）删除输入的临时文件
     *
    ...
    public static String handleAvatar(String srcImageFilePath, int x, int y,
        ...
        compressStandardList.add(ImageUploadUtil.IMAGE_COMPRESS_STANDARD_200);
        compressStandardList.add(ImageUploadUtil.IMAGE_COMPRESS_STANDARD_120);
        compressStandardList.add(ImageUploadUtil.IMAGE_COMPRESS_STANDARD_30);
        ...
        for (Integer compressStandard : compressStandardList) {
            String newFilePath = ImageUploadUtil.appendImageSizePostfix(
                    ...
                    compressStandard);
            ImageUploadUtil.compressImageConstrain(cutImageFilePath,
