...
public class DateUtil {
	...
	public static long getNow(String zone, String locale, int preday) {
		...
		ca.setTimeInMillis(new java.util.Date().getTime() - (long) preday * 24
				* 60 * 60 * 1000);
	...
	public static String preHours(int hour, DateFormat format) {
		...
		Calendar ca = Calendar.getInstance();
		ca.clear();
		ca.setTimeInMillis(new java.util.Date().getTime() - (long) hour * 60
				* 60 * 1000);
		return Format(format, ca.getTime());
	}
...
//			String message = "format is null,the dataString is " + dateString;
	 ...
	 * @param week
	 * @param isFirst
	 *            true为求出入周的开始日期，false为求last日期
	 * @param m
	 *            日期是否是以星期一开始,否的话,默认为星期日为一周的开始日期<br>
	 * @return isMonday
	 */
	public static String getDayOfWeek(int year, int week, boolean isFirst,
			boolean isMonday) {
	...
	public static String preMins(DateFormat format, int min) {
		Calendar ca = Calendar.getInstance();
		ca.clear();
		ca.setTimeInMillis(new java.util.Date().getTime() - (long) min * 60
				* 1000);
		return Format(format, ca.getTime());
	}
	...
	public static java.sql.Date utilDate_to_sqlDate(java.util.Date utilDate,
			boolean need_default) {
		if (utilDate == null) {
			if (need_default) {
