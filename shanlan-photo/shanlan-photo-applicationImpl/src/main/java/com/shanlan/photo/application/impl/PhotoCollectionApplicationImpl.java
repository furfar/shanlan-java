...
@Named
@Transactional
public class PhotoCollectionApplicationImpl implements
        PhotoCollectionApplication {
    ...
    public PhotoCollectionDTO getPhotoCollection(Integer id) {
        PhotoCollection photoCollection = PhotoCollection.load(
                PhotoCollection.class, id);
    ...
    public PhotoCollectionDTO savePhotoCollection(
            PhotoCollectionDTO photoCollectionDTO) {
    ...
    public void updatePhotoCollection(PhotoCollectionDTO photoCollectionDTO) {
        PhotoCollection photoCollection = PhotoCollection.get(
                PhotoCollection.class, photoCollectionDTO.getId());
        photoCollection.setUpdatedAt(DateUtil
                .getNow(DateUtil.format_yyyyMMdd_HHmmss));
        photoCollection.setName(photoCollectionDTO.getName());
        photoCollection.setDescription(photoCollectionDTO.getDescription());
        photoCollection.save();
    }
    ...
    public List<PhotoCollectionDTO> findAllPhotoCollection() {
        ...
        List<PhotoCollection> all = PhotoCollection
                .findAll(PhotoCollection.class);
    ...
    @Transactional(propagation = Propagation.SUPPORTS, readOnly = true)
    public Page<PhotoCollectionDTO> pageQueryPhotoCollection(
            PhotoCollectionDTO queryVo, int currentPage, int pageSize) {
        List<PhotoCollectionDTO> result = new ArrayList<PhotoCollectionDTO>();
        List<Object> conditionVals = new ArrayList<Object>();
        StringBuilder jpql = new StringBuilder(
                "select _photoCollection from PhotoCollection _photoCollection   where 1=1 ");
        ...
        Page<PhotoCollection> pages = getQueryChannelService()
        ...
        return new Page<PhotoCollectionDTO>(pages.getStart(),
    ...
    public Page<PhotoCollectionDTO> pageQueryPhotoCollection(
            PhotoCollectionDTO queryVo, int currentPage, int pageSize,
            String userName, boolean isSuper, List<String> roles) {
        List<PhotoCollectionDTO> result = new ArrayList<PhotoCollectionDTO>();
        List<Object> conditionVals = new ArrayList<Object>();
        StringBuilder jpql = new StringBuilder(
                "select _photoCollection from PhotoCollection _photoCollection   where 1=1 ");
    ...
    @Override
    public boolean updatePhotoCount(Integer photoCollectionId) {
        Integer photoCount = RePhotoCollectionPhoto.findByPhotoCollectionId(
                photoCollectionId).size();
        return PhotoCollection.updatePhotoCount(photoCollectionId, photoCount);
    }
}
